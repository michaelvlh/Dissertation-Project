...
new_best_path = "../trained_models/chroma/re_best_model.pth"

# path to save testing results
res_path = "../Results/chroma/testResult.txt"
...
if os.path.isfile(model_path):
    ...
    print("start tuning")
    chromaModel.chromaTuning()
    print("finish tuning")
    trained = torch.load(model_path)
    params = trained["hyper_params"]

# setup transform function
chroma_transformer = dataLoad.chromaTransform(n_fft=params['nfft'])

# setup dataset and data loader
# Training set
realDataset = dataLoad.getData(tRealPath, frac=1, transform=chroma_transformer)
fakeDataset = dataLoad.getData(tFakePath, frac=1, transform=chroma_transformer)
trainDataset = torch.utils.data.ConcatDataset([realDataset, fakeDataset])
# Validation set
vrealDataset = dataLoad.getData(vRealPath, frac=1, transform=chroma_transformer)
vfakeDataset = dataLoad.getData(vFakePath, frac=1, transform=chroma_transformer)
valDataset = torch.utils.data.ConcatDataset([vrealDataset, vfakeDataset])
# Test set
realSet = dataLoad.getData(realPath, frac=1, transform=chroma_transformer)
fakeSet = dataLoad.getData(fakePath, frac=1, transform=chroma_transformer)
