...
model_path = "../trained_models/mfcc/best_model.pth"
new_best_path = "../trained_models/mfcc/re_best_model.pth"

# path to save testing results
res_path = "../Results/mfcc/testResult.txt"

# best predictions
best_pred_path = "../Results/mfcc/"

# graph directory
image_path = "../image/mfcc/"
...
if os.path.isfile(model_path):
    ...
    print("start tuning")
    mfccModel.mfccTuning()
    print("finish tuning")
    trained = torch.load(model_path)
    params = trained["hyper_params"]

# setup transform function
mfcc_transformer = torchaudio.transforms.MFCC(n_mfcc=params['n_mels'], melkwargs={"n_fft": params['nfft'],
                                                                                      "n_mels": 128,
                                                                                      "hop_length": 512,
                                                                                      "mel_scale": "htk",
                                                                                      },)
...
# Training set
realDataset = dataLoad.getData(tRealPath, frac=1, transform=mfcc_transformer)
fakeDataset = dataLoad.getData(tFakePath, frac=1, transform=mfcc_transformer)
trainDataset = torch.utils.data.ConcatDataset([realDataset, fakeDataset])
# Validation set
vrealDataset = dataLoad.getData(vRealPath, frac=1, transform=mfcc_transformer)
vfakeDataset = dataLoad.getData(vFakePath, frac=1, transform=mfcc_transformer)
valDataset = torch.utils.data.ConcatDataset([vrealDataset, vfakeDataset])
# Test set
realSet = dataLoad.getData(realPath, frac=1, transform=mfcc_transformer)
fakeSet = dataLoad.getData(fakePath, frac=1, transform=mfcc_transformer)
